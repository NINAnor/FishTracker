...
def main():
    ...
    for i, detections in enumerate(all_detections):
        ...
        for detection in detections:
            cv2.circle(
                image_o_rgb, tuple(list(detection.astype(int))), 3, (0, 128, 255), -1
            )
        ...
        for t, track in enumerate(tracker.tracks):
            cv2.putText(
                image_o_rgb,
                str(track.consecutive_updates),
                tuple(list(track.last_position.astype(int))),
            ...
            for t in track.history:
                ...
                if track.status == "Tentative":
                    cv2.line(
                        image_o_rgb,
                        tuple(list(previous.astype(int))),
                        tuple(list(t.astype(int))),
                        (128, 128, 128),
                        2,
                    )
