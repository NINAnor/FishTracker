...
class FishList(QtWidgets.QWidget):
    ...
    def __init__(self, fish_manager, playback_manager, sonar_viewer=None):
        ...
        self.fish_manager.updateSelectionSignal.connect(
            self.table.selectionModel().select
        )
        self.fish_manager.updateSelectionSignal.connect(
            lambda x, y: self.table.setFocus()
        )
        ...
        self.min_detections_slider = LabeledSlider(
            "Min duration",
            self.form_layout,
            [self.fish_manager.setMinDetections],
            self.fish_manager.min_detections,
            1,
            50,
            self,
        )
        self.mad_slider = LabeledSlider(
            "MAD",
            self.form_layout,
            [self.fish_manager.setMAD],
            self.fish_manager.mad_limit,
            0,
            50,
            self,
        )
        self.length_percentile_slider = LabeledSlider(
            "Length percentile",
            self.form_layout,
            [self.fish_manager.setLengthPercentile],
            self.fish_manager.length_percentile,
            1,
            100,
            self,
        )
        ...
        self.display_btn.setToolTip(
            "Go to the frame where the selected fish is first detected."
        )
        ...
        self.measure_btn.setToolTip(
            "Draw a line on Sonar View to measure the length of the selected fish."
        )
        ...
        self.clear_measure_btn.setToolTip(
            "Clear measured lengths on all selected fish."
        )
        ...
        self.split_btn.setToolTip(
            "Split all selected fish in two at the current frame."
        )
    ...
    def updateCountLabels(self):
        ...
        self.total_fish_label.setText(f"Total: {tc}")
        self.up_fish_label.setText(f"Up: {uc}")
        self.down_fish_label.setText(f"Down: {dc}")
        self.none_fish_label.setText(f"None: {nc}")
...
if __name__ == "__main__":
    ...
    def dataTest():
        ...
        playback_manager.mapping_done.connect(
            lambda: playback_manager.runInThread(startDetector)
        )
