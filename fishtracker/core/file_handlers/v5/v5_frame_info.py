...
JSON_FILE_PATH = Path(__file__).parent / "v5_frame_headers_info.json"
        ...
        try:
            with open(filename, "rb") as fhand:
                ...
                self.latitude = struct.unpack(
                    utils.cType["double"], fhand.read(utils.c("double"))
                )[0]
                ...
                self.ARISAppVersionMajor = struct.unpack(
                    utils.cType["uint16_t"], fhand.read(utils.c("uint16_t"))
                )[0]
                ...
                self.padding = struct.unpack(
                    utils.cType["char[288]"], fhand.read(utils.c("char[288]"))
                )[0]
                ###########################
                # Functions Section
                ###########################
    ...
    def getInfo(self, all=False):
        ...
        if all:
            ...
            info = {
                ...
                "Frame Time": datetime.datetime.fromtimestamp(
        ...
        for rows in range(len(data)):
            for cols in range(len(data[rows])):
                data[rows][cols] = struct.unpack(
                    utils.cType["uint8_t"], fileHandle.read(1)
                )[0]
