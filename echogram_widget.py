...
from fish_manager import FishEntry, pyqt_palette
from log_object import LogObject
from zoomable_qlabel import DebugZQLabel, ZoomableQLabel
...
class EchoFigure(ZoomableQLabel):
    ...
    def frame2xPos(self, value):
        ...
        try:
            return (
                (value * self.image_width - self.x_min_limit)
    ...
    def xPos2Frame(self, value):
        ...
        try:
            return (
                value * (self.x_max_limit - self.x_min_limit) / self.window_width
                + self.x_min_limit
            ) / self.image_width
        except ZeroDivisionError as e:
            LogObject().print(e)
    ...
    def paintEvent(self, event):
        ...
        if (
            self.parent.detector._show_echogram_detections
            or self.parent.fish_manager.show_echogram_fish
        ):
    ...
    def overlayPixmap(self, painter, pixmap):
        ...
        if pixmap is not None and shown_frames > 0 and current_frames > 0:
            ...
            source = QtCore.QRectF(
                source_L, 0, self.shown_width - source_R - source_L, self.shown_height
            )
            target = QtCore.QRectF(
                target_L, 0, self.window_width - target_R - target_L, self.window_height
            )
            ...
            if self.debug_lines:
                ...
                painter.drawLine(
                    QtCore.QLineF(target_L, 0, target_L, self.window_height)
                )
                painter.drawLine(
                    QtCore.QLineF(
                        self.window_width - target_R,
                        0,
                        self.window_width - target_R,
                        self.window_height,
                    )
    ...
    def updateDetectionImage(self, painter, vertical_detections):
        ...
        try:
            for i, heights in enumerate(
                vertical_detections[self.x_min_limit : self.x_max_limit]
            ):
    ...
    def updateTrackImage(self, painter, vertical_tracks):
        ...
        try:
            for i, height_color_pairs in enumerate(
                vertical_tracks[self.x_min_limit : self.x_max_limit]
            ):
                h_pos_1 = (i + 1) / show_frame_count * self.window_width
...
class EchogramViewer(QtWidgets.QWidget):
    ...
    def __init__(self, playback_manager, detector, fish_manager):
        ...
        self.playback_manager.polars_loaded.connect(
            lambda: self.playback_manager.runInThread(self.processEchogram)
        )
    ...
    def updateVerticalTracks(self):
        ...
        self.vertical_tracks = [
            [] for fr in range(self.playback_manager.getFrameCount())
        ]
    ...
    def updateOverlayedImage(self):
        self.figure.updateOverlayedImage(
            self.detector.vertical_detections, self.vertical_tracks
        )
        self.figure.update()
    ...
    def onBoxSelect(self, box_positions):
        ...
        min_d, max_d = self.playback_manager.getRadiusLimits()
        height_max = min_d + max(0, (1 - min_y / self.figure.image_height)) * (
            max_d - min_d
        )
        height_min = min_d + min((1 - max_y / self.figure.image_height), 1) * (
            max_d - min_d
        )

        self.fish_manager.selectFromEchogram(
            frame_min, frame_max, height_min, height_max
        )
...
class Echogram:
    ...
    def processBuffer(self, buffer):
        ...
        try:
            ...
            self.bgs_data = np.squeeze(
                [self.bg_subtractor.subtractBG(column) for column in self.data]
            )
...
if __name__ == "__main__":
    ...
    class TestDetector(QtCore.QObject):
        ...
        def onPolarsLoaded(self):
            ...
            for i in range(self.frameCount):
                ...
                if count > 0:
                    self.detections.append(
                        [
                            TestDetection(0, np.random.uniform(min_r, max_r))
                            for j in range(count)
                        ]
                        + [TestDetection(0, min_r), TestDetection(0, max_r)]
                    )
                else:
            ...
            self.vertical_detections = [
                [d.center[0] for d in dets if d.center is not None]
                if dets is not None
                else []
