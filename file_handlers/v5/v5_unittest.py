...
class v5_DIDSON_FILE_TEST(unittest.TestCase):
    test_param = {
        "filePath": {
            ...
            "absGood": os.path.abspath(
                os.path.join("file_handlers", "v5", "sample.aris")
            ),
        },
        "corruptedFilePath": os.path.join(
            "file_handlers", "v5", "corruptedSample.aris"
        ),
        "version": 88491076,
        "frameCount": 6,
        "frameRate": 0,
        ...
        "reverse": 0,
        "serialNumber": 1098,
        "strDate": b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
        b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
        "strHeaderID": b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
        b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
        b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
        b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
        b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
    ...
    def testFrameCount(self):
        f = v5.v5_File(self.test_param["filePath"]["good"])
        self.assertEqual(self.test_param["frameCount"], len(f))
        return

    def testFilePath(self):
        f = v5.v5_File(self.test_param["filePath"]["good"])
        self.assertEqual(self.test_param["filePath"]["absGood"], repr(f))
        return

    def testFileName(self):
        f = v5.v5_File(self.test_param["filePath"]["good"])
        self.assertEqual(self.test_param["filePath"]["name"], f.getFileName())
        return
